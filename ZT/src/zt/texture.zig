...
pub fn init(filePath: []const u8) !Self {
    ...
    std.log.debug("{} channels", .{numChannels});
    switch (numChannels) {
        3 => {
    ...
    // gl.glPixelStorei(gl.GL_UNPACK_ALIGNMENT, 1); --> we put this into our texturecache.zig to leave this zt source file intact;
...
pub fn initMemory(slice: []const u8) !Self {
    ...
    switch (numChannels) {
        3 => {
            gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, gl.GL_RGB, w, h, 0, gl.GL_RGB, gl.GL_BYTE, data);
        },
        4 => {
            gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, gl.GL_RGBA, w, h, 0, gl.GL_RGBA, gl.GL_BYTE, data);
        },
        else => {
