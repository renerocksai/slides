...
// call context.rebuildFont() after this
pub fn loadFonts() error{OutOfMemory}!void {
    ...
    // commitFonts(); --> zt.App.rebuildFont()
}
...
pub fn addZigShowtimeFont() !void {
    ...
    // var font = imgui.ImFontAtlas_AddFontFromMemoryTTF(io.*.Fonts, fontdata_zigshowtimefont, fontdata_normal.len, @intToFloat(f32, fsize), font_config, imgui.ImFontAtlas_GetGlyphRangesDefault(io.*.Fonts));
...
pub fn addFont(fsize: i32) !void {
    ...
    font_config[0].OversampleV = 1;
    font_config[0].FontDataOwnedByAtlas = false;
    var font = imgui.ImFontAtlas_AddFontFromMemoryTTF(io.*.Fonts, @intToPtr(*anyopaque, @ptrToInt(fontdata_normal)), fontdata_normal.len, @intToFloat(f32, fsize), font_config, &my_font_ranges);
    // see above: bullet range only needs to be applied to default font
    try my_fonts.put(fsize, font);

    var font_config_bold = imgui.ImFontConfig_ImFontConfig();
    ...
    var font_bold = imgui.ImFontAtlas_AddFontFromMemoryTTF(io.*.Fonts, castaway_const(fontdata_bold), fontdata_bold.len, @intToFloat(f32, fsize), font_config_bold, imgui.ImFontAtlas_GetGlyphRangesDefault(io.*.Fonts));
    try my_fonts_bold.put(fsize, font_bold);

    var font_config_italic = imgui.ImFontConfig_ImFontConfig();
    ...
    var font_italic = imgui.ImFontAtlas_AddFontFromMemoryTTF(io.*.Fonts, castaway_const(fontdata_italic), fontdata_italic.len, @intToFloat(f32, fsize), font_config_italic, imgui.ImFontAtlas_GetGlyphRangesDefault(io.*.Fonts));
    try my_fonts_italic.put(fsize, font_italic);

    var font_config_bolditalic = imgui.ImFontConfig_ImFontConfig();
    ...
    var font_bolditalic = imgui.ImFontAtlas_AddFontFromMemoryTTF(io.*.Fonts, castaway_const(fontdata_bolditalic), fontdata_bolditalic.len, @intToFloat(f32, fsize), font_config_bolditalic, imgui.ImFontAtlas_GetGlyphRangesDefault(io.*.Fonts));
    try my_fonts_bolditalic.put(fsize, font_bolditalic);

    // special case: our zig font
    std.log.debug("trying {d}", .{fsize});
    if (fsize < 192) {
        var font_config_zig = imgui.ImFontConfig_ImFontConfig();
        ...
        var font_zig = imgui.ImFontAtlas_AddFontFromMemoryTTF(io.*.Fonts, castaway_const(fontdata_zig), fontdata_zig.len, @intToFloat(f32, fsize), font_config_zig, imgui.ImFontAtlas_GetGlyphRangesDefault(io.*.Fonts));
...
pub fn getFontScaled(pixels: i32) bakedFontInfo {
    ...
    var font: *imgui.ImFont = my_fonts.get(baked_font_sizes[0]).?; // we don't ever down-scale. hence, default to minimum font size
