...
int zipIt(char *zipFileName, ZipFnPair *fnPairs, int count)
{
...
#        endif
    ...
    int i = 0;
    for (i=0;(i<count) && (err==ZIP_OK);i++)
    {
        FILE * fin;
        size_t size_read;
        const char* filenameinzip = fnPairs[i].fnOnDisk;
        const char *savefilenameinzip = fnPairs[i].fnInZip;
        zip_fileinfo zi;
